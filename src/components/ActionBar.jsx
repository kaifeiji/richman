...
export default function ActionBar({ game, animation, paused = false, canAct = true, onRoll, onBuy, onBuild, onEnd, onRelease, onDraw, onMechanismRoll, onResolve }) {
  ...
  if (animation.moving) { title = '移动中'; description = ''; }
    ...
    const resolveTitles = { card: '确认', payment: '支付费用', tax: '缴纳税款', environment: '支持环保', market: '结算盈亏', arrest: '本回合暂停', harbor: '安心停靠' };
  ...
  else if (canBuy) { title = `买下 ${tile.name}？`; description = `取得所有权 · ${money(tile.ownership)} · 按 +` ; }
  else if (canBuild) { title = `盖 ${level + 1} 层楼`; description = `${tile.name} · ${money(tile.buildCost)} · 按 +`; }
