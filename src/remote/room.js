...
export function connectRoom({ roomName, nickname, clientId, onMessage, onOpen, onClose, onError }) {
  const socket = new WebSocket(`${SIGNAL_URL}?room=${encodeURIComponent(roomName)}&name=${encodeURIComponent(nickname)}&client=${encodeURIComponent(clientId)}`);
